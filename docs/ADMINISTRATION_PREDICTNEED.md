...
Les automatisations email actives doivent être rattachées à un `SiteClient`. Les anciennes lignes `AutomatisationEmail.site=NULL` sont conservées pour audit historique, mais le scheduler de relances les ignore : elles ne déclenchent aucun email client-wide et ne mélangent pas silencieusement les leads de plusieurs sites. Pour réactiver une ancienne règle, il faut créer ou rattacher explicitement une automatisation au site concerné.
